...
function LoginForm() {
  ...
  const [error, setError] = useState(params.get('error') === 'unauthorized' ? 'Kein Admin-Zugang' : '')
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="glass rounded-2xl p-8 w-full max-w-md">
        <h1 className="font-serif text-3xl font-bold text-gold text-center mb-2">Da Sergio</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {!needsNewPassword ? (
            <>
              <div>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                  className="w-full px-4 py-3 rounded-lg bg-deep-green-dark border border-border focus:border-gold text-foreground focus:outline-none" />
              </div>
              <div>
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">Neues Passwort festlegen</label>
